...
namespace HistoGrading.Components
{
    ...
    public class Processing
    {
        ...
        public static vtkImageData SWFPSuppression(vtkImageData BCI, int[] extent, double threshold = 0.7 * 255.0, int min = 0, int max = 650, int step = 4, int ws = 10, double wt = 4.0, double minones = 192.0)
        {
            //Get dimensions
            int[] dims = BCI.GetExtent();
            ...
            for (int k1 = 0; k1 < N; k1++)
            {
                ...
                {
                    int step1 = (extent[1] - extent[0] + 1) / N;
                    int step2 = (extent[3] - extent[2] + 1) / N;
